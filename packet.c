...
{
	int pkt_len=IPHDR_SIZE+ICMPHDR_SIZE, err;
	...
	ip_pkt->id = 0; /* kernel sets proper value htons(ip_id_counter); (Windows requires "0", otherwise it will stay as is) */
	...
	pkt = (struct icmp_packet_t*)(packet+IPHDR_SIZE);
	pkt->type = server ? 8 : 11; /* ICMP echo request or time exceeded */
	pkt->code = 0; /* Must be zero */
	pkt->identifier = server?htons(1):htons(IPHDR_SIZE+ICMPHDR_SIZE);/* identifier might be length of original datagram... normally unused */
//	pkt->identifier = server?htons(1):htons(0);/* identifier might be length of original datagram... normally unused */
	pkt->seq = server?htons(1337):0;
	if(!server){/* Generate "original" packet if client to append to time exceeded */
		ip_pkt2 = (struct ip_packet_t*)(packet+IPHDR_SIZE+ICMPHDR_SIZE);
...
/*		ip_pkt2->pkt_len = (IPHDR_SIZE+ICMPHDR_SIZE) << 8; */
		...
		ip_pkt2->id = 0; /* kernel sets proper value htons(ip_id_counter); (Windows requires "0", otherwise it will stay as is) */
		ip_pkt2->flags_frag_offset = 0;
		ip_pkt2->ttl = 1; /* real TTL would be 1 on a time exceeded packet */
		ip_pkt2->proto = 1; /* ICMP */
		ip_pkt2->checksum = 0; /* we have to calculate it */
		...
		pkt2 = (struct icmp_packet_t*)(packet+IPHDR_SIZE+ICMPHDR_SIZE+IPHDR_SIZE);
		...
		pkt->checksum = 0;
		pkt->checksum = htons(calc_icmp_checksum((uint16_t*)pkt, ICMPHDR_SIZE+IPHDR_SIZE+ICMPHDR_SIZE));
	}else{/* just calculate checksum */
		pkt->checksum = 0;
		pkt->checksum = htons(calc_icmp_checksum((uint16_t*)pkt, ICMPHDR_SIZE));
	}
	/*FILE* fp=fopen("_pack","wb");
	fwrite(packet,pkt_len,1,fp);
	fclose(fp);*/
	...
	if(err < 0) {
		...
		puts("WARNING WARNING, didn't send entire packet");
	return 0;
}
